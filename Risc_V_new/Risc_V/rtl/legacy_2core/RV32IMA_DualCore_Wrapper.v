...
module RV32IMA_DualCore_Wrapper(
    input  wire        clk,
    input  wire        rst,
    ...
    // synth nhầm RV32IMA bằng stub không có parameter RESET_ADDR.
    // ============================================================
    RV32IMA core0 (
        .clk                (clk),
        .rst                (rst),
        ...
        // NEW (RV32IMA.v now has a trap/CSR unit -- see rtl/csr_trap_unit.v):
        // this design has no MMU in front of it, so there is no page
        // fault to report -- tied low to preserve this file's exact
        // original behaviour. This is the one required exception to
        // "don't touch the legacy 2-core files": RV32IMA.v gained two
        // new inputs this session, and leaving them unconnected would
        // float (X in simulation) into csr_trap_unit.v's exception
        // logic and could spuriously trap -- silently breaking this
        // still-simulatable fallback path, not just leaving it alone.
        // The 4 new debug/status outputs (CurrentPriv, Mmu_Enable_Csr,
        // Satp_PPN_Csr, Mmu_Flush_Csr) are left unconnected -- legal
