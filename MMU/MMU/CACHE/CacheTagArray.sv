module CacheTagArray #(
    ...
    reg [TAG_WIDTH-1:0] tags  [0:NUM_SETS-1][0:1];
    reg                 valid [0:NUM_SETS-1][0:1];
    reg                 dirty [0:NUM_SETS-1][0:1];
    ...
    always_ff @(posedge clk or posedge reset) begin
        if (reset) begin
            for (set_idx = 0; set_idx < NUM_SETS; set_idx = set_idx + 1) begin
                tags[set_idx][0]  <= '0;
                tags[set_idx][1]  <= '0;
                valid[set_idx][0] <= 1'b0;
                valid[set_idx][1] <= 1'b0;
                dirty[set_idx][0] <= 1'b0;
                dirty[set_idx][1] <= 1'b0;
            end
