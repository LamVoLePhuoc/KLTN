module D_Cache (
    ...
    // Phase-2 coherence: pulse snoop_en with the byte address of a line
    // another core just wrote through to L2; this cache invalidates its
    // copy of that line if it has one cached.
    input          snoop_en,
    ...
    wire [4:0]  snoop_index = snoop_addr[9:5];
    wire [21:0] snoop_tag   = snoop_addr[31:10];
    ...
    TagArray d_tag_array (
        ...
        .snoop_en(snoop_en), .snoop_index(snoop_index), .snoop_tag(snoop_tag)
    );
